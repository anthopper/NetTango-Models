...
buildings-own [strength-of-building startingstrength]
globals [
...
to setup ; set upm the model
  ...
  ask buildings [set strength-of-building 50 set startingstrength strength-of-building ]
  reset-ticks
end
...
to setup-house [x y health]
  ask patch x y [if not any? other buildings in-radius 4 [sprout-buildings 1 [set shape "house" set size 5 set color blue set strength-of-building health set startingstrength strength-of-building ]]]
end

to placehouse
  if mouse-down? [setup-house  mouse-xcor mouse-ycor health-points]
...
to go
  ...
  ask buildings
  [
    set label word "health points: " precision
    strength-of-building 0
    damage
    shake
  ]
...
to earthquake [col magnitute] ; starts a quake near fault lines which are color coded from the imported picture file
  ask n-of 2 wavers with [pcolor = col]
